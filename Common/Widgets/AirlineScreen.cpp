...
AirlineScreen::AirlineScreen (Classification::Definitions *d, QWidget *p, Qt::WindowFlags f)
  : QDialog (p, f)
  , m_definitions (d)
  , m_data_mapper (new QDataWidgetMapper (this))
  , m_model (d->airlineModel())
  ...
  , m_libgvc_engine (new QComboBox)
{
  ...
  connect (m_model, &Classification::AirlineModel::modelReset,
           this, &AirlineScreen::relayout);
  connect (m_tree, &AirlineGraphWidget::nodeClicked,
...
AirlineScreen::createAirlineBox ()
{
  ...
  connect (icao, SIGNAL(currentIndexChanged(int)), m_data_mapper, SLOT(setCurrentIndex(int)));
  connect (icao, SIGNAL(currentIndexChanged(int)), this, SLOT(updateSelected()));
